...
if __name__ == "__main__":
    ...
    for algo, exp_folder in zip(list_algo, list_exp_folders):
        ...
        plt.plot(
            np.arange(1, parms.al_iter // parms.test_interval + 1)
            * parms.test_interval,
            mean_test_losses,
            label=algo,
        )
    ...
    for algo, exp_folder in zip(list_algo, list_exp_folders):
        test_losses_by_seed = pickle.load(
            open(
                os.path.join(RESULTS_DIR, exp_folder, "average_observed_y_by_seed.pkl"),
                "rb",
            )
        )
    ...
    for algo, exp_folder in zip(list_algo, list_exp_folders):
        test_losses_by_seed = pickle.load(
            open(
                os.path.join(
                    RESULTS_DIR, exp_folder, "percentage_observed_top1_by_seed.pkl"
                ),
                "rb",
            )
        )
    ...
    for algo, exp_folder in zip(list_algo, list_exp_folders):
        test_losses_by_seed = pickle.load(
            open(
                os.path.join(
                    RESULTS_DIR, exp_folder, "percentage_observed_top5_by_seed.pkl"
                ),
                "rb",
            )
        )
    ...
    for algo, exp_folder in zip(list_algo, list_exp_folders):
        test_losses_by_seed = pickle.load(
            open(
                os.path.join(
                    RESULTS_DIR, exp_folder, "percentage_observed_top10_by_seed.pkl"
                ),
                "rb",
            )
        )
