...
def train(model: nn.Module, loss_fct: Callable, x: torch.Tensor, y: torch.Tensor,
          batch_size: int = 1, optim: Optimizer = None, burnin_iter: int = 0, mcmc_iter: int = 50, glr: str = 'var',
          weight_decay: float = 0., lr: float = 1e-3, num_epochs: int = 1000, gamma: float = 0.996) -> (list):

    losses, models = [], []
    if optim is not None:
        optim = optim(model.parameters(), lr=lr,
                      weight_decay=weight_decay, glr=glr)
    else:
    ...
    for epoch in pbar:
        ...
        for batch_idx in range(1, m+1):
            ...
            out = model(
                x[idx[(batch_idx-1) * batch_size: batch_idx * batch_size]])
...
def predict(models: [nn.Module], x: torch.Tensor) -> (torch.Tensor):
