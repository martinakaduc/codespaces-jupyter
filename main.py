...
if __name__ == "__main__":
    ...
    # Run experiments
    test_losses_by_seed = {x: [] for x in parms.seeds}
    percentage_observed_top1_by_seed = {x: [] for x in parms.seeds}
    percentage_observed_top5_by_seed = {x: [] for x in parms.seeds}
    percentage_observed_top10_by_seed = {x: [] for x in parms.seeds}
    average_observed_y_by_seed = {x: [] for x in parms.seeds}
    
    def run_by_seed(seed, 
                    test_losses_by_seed, 
    ...
    pickle.dump(percentage_observed_top1_by_seed, open(
        os.path.join(plot_dir, "percentage_observed_top1_by_seed.pkl"), "wb"))

    pickle.dump(percentage_observed_top5_by_seed, open(
        os.path.join(plot_dir, "percentage_observed_top5_by_seed.pkl"), "wb"))

    pickle.dump(percentage_observed_top10_by_seed, open(
        os.path.join(plot_dir, "percentage_observed_top10_by_seed.pkl"), "wb"))
    
    pickle.dump(average_observed_y_by_seed, open(
        os.path.join(plot_dir, "average_observed_y_by_seed.pkl"), "wb"))
