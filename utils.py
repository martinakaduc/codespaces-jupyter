...
def calc_uncert(
    preds: List[torch.Tensor], reduction: str = "mean"
...
def plot_uncert(
    ...
    x_test, y_pred_mean, x_train, y_train = (
        x_test.cpu(),
        y_pred_mean.cpu(),
        x_train.cpu(),
        y_train.cpu(),
    ...
    ax.scatter(x_train[:-1], y_train[:-1], color="black", label="Training data")
    ax.scatter(x_train[-1:], y_train[-1:], color="red", label="Next point")
